...
/*	1)	Add "Boilerplate Admin" link to left-nav Admin Menu & callback function for clicking that menu link */
	...
	if ( ! function_exists( 'create_boilerplate_admin_page' ) ):
		function create_boilerplate_admin_page() {
			add_theme_page( __('General Options', 'castroytagle'), __( 'General Options', 'castroytagle'), 'administrator', 'boilerplate-admin', 'build_boilerplate_admin_page');
		}
	...
	if ( ! function_exists( 'build_boilerplate_admin_page' ) ):
		...
		?>
			<div id="boilerplate-options-wrap">
				...
				<h2><?php _e( 'General options for', 'castroytagle' ); ?> <?php echo wp_get_theme() . __( '', 'castroytagle' );?> </h2>
				<p><?php _e( 'This is the web site manager. Select the options you want to include on the website.', 'castroytagle' ); ?></p>
...
/*	3)	Add "Boilerplate Admin" Page options */
	...
	if ( ! function_exists( 'google_verification_setting' ) ):
		function google_verification_setting() {
			...
			echo _e( '<p>Add <a href="http://www.google.com/support/webmasters/bin/answer.py?answer=35179">Google Verificaton</a> code and <a href="https://www.google.com/analytics">Google Analytics</a> to the <code>&lt;head&gt;</code> of all your pages.</p>', 'castroytagle' );
			echo _e( '<p>To include Google Verificaton and Google Analytics, select this option and include your Verificaton number here:</p><br />', 'castroytagle' );
			...
			echo _e( '<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages, where <code>XXXXXXXXX?</code> will be replaced with the code you insert above</p>', 'castroytagle' );
			echo '<code>&lt;meta name="google-site-verification" content="'.$account.'"&gt;</code>';
			echo _e( 'And will add the latest version of the Analytics tracking code, to track website visitors with Google Analytics.', 'castroytagle' );
		}
	...
	if ( ! function_exists( 'viewport_setting' ) ):
		function viewport_setting() {
			...
			echo _e( '<p>Force <em><abbr title="iPhone, iTouch, iPad...">iThings</abbr></em> to <a href="http://developer.apple.com/library/safari/#documentation/AppleApplications/Reference/SafariWebContent/UsingtheViewport/UsingtheViewport.html#//apple_ref/doc/uid/TP40006509-SW19">show site at full-zoom</a>, instead of trying to show the entire page.</p>', 'castroytagle' );
			echo _e( '<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages:</p>', 'castroytagle' );
	...
	if ( ! function_exists( 'favicon_setting' ) ):
		function favicon_setting() {
			...
			echo _e( '<p>If you plan to use a <a href="http://en.wikipedia.org/wiki/Favicon">favicon</a> for your site, place the "favicon.ico" file in the root directory of your site.</p>', 'castroytagle' );
			echo _e( '<p>If the file is in the right location, you don\'t really need to select this option, browsers will automatically look there and no additional code will be added to your pages.</p>', 'castroytagle' );
			echo _e( '<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages:</p>', 'castroytagle' );
	...
	if ( ! function_exists( 'modernizr_js_setting' ) ):
		function modernizr_js_setting() {
			...
			echo _e('<p><a href="http://modernizr.com">Modernizr</a> is a JS library that appends classes to the <code>&lt;html&gt;</code> that indicate whether the user\'s browser is capable of handling advanced CSS, like "cssreflections" or "no-cssreflections".  It\'s a really handy way to apply varying CSS techniques, depending on the user\'s browser\'s abilities, without resorting to CSS hacks.</p>', 'castroytagle' );
			echo _e('<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages (note the lack of a version, when you\'re ready to upgrade, simply copy and paste the new version into the file below, and your site is ready to go!):</p>', 'castroytagle' );
			echo '<code>&lt;script src="' .BP_THEME_URL. '/js/modernizr.js"&gt;&lt;/script&gt;</code>';
			echo _e('<p><strong>Note: If you do <em>not</em> include Modernizr, the IEShiv JS <em>will</em> be added to accommodate the HTML5 elements used in Boilerplate in weaker browsers:</strong></p>', 'castroytagle' );
	...
	if ( ! function_exists( 'jquery_js_setting' ) ):
		function jquery_js_setting() {
			...
			$version = (isset($options['jquery_version']) && $options['jquery_version'] && $options['jquery_version'] !== '') ? $options['jquery_version'] : '1.9.1';
			...
			echo _e('<p><a href="http://jquery.com/">jQuery</a> is a JS library that aids greatly in developing high-quality JavaScript quickly and efficiently.</p>', 'castroytagle' );
			echo  _e( '<p>Selecting this option will add the following code to your pages just before the: <code>&lt;/body&gt;</code></p>', 'castroytagle' );
			echo '<code>&lt;script src="//ajax.googleapis.com/ajax/libs/jquery/'.$version.'/jquery.min.js">&lt;/script&gt;</code>';
			echo '<code>&lt;script&gt;window.jQuery || document.write("&lt;script src="js/vendor/jquery-1.8.0.min.js"&lt;/script&gt;")</code>';
			echo '<p><input class="check-field" type="checkbox" name="plugin_options[jquery_head]" value="true" ' .$inhead. '/>';
			echo _e('<p><strong>Note: <a href="http://developer.yahoo.com/blogs/ydn/posts/2007/07/high_performanc_5/">Best-practices</a> recommend that you load JS as close to the <code>&lt;/body&gt;</code> as possible.  If for some reason you would prefer jQuery and jQuery plug-ins to be in the <code>&lt;head&gt;</code>, please select this option.</strong></p>', 'castroytagle' );
			echo _e('<p>The above code first tries to download jQuery from Google\'s CDN (which might be available via the user\'s browser cache).  If this is not successful, it uses the theme\'s version.</p>', 'castroytagle' );
			echo _e('<p><strong>Note: This plug-in tries to keep current with the most recent version of jQuery.  If for some reason you would prefer to use another version, please indicate that version:</strong></p><br/>', 'castroytagle' );
	...
	if ( ! function_exists( 'site_js_setting' ) ):
		function site_js_setting() {
			...
			echo _e( '<p>If you would like to add your own site JavaScript file, we provides a starter file located in:</p>', 'castroytagle' );
			echo '<code>' .BP_THEME_URL. '/js/script-starter.js</code>';
			echo _e( '<p>Add what you want to that file and select this option.</p>', 'castroytagle' );
			echo  _e( '<p>Selecting this option will add the following code to your pages just before the: <code>&lt;/body&gt;</code></p>', 'castroytagle' );
	...
	if ( ! function_exists( 'cache_buster_setting' ) ):
		function cache_buster_setting() {
			...
			echo _e( '<p>To force browsers to fetch a new version of a file, versus one it might already have cached, you can add a "cache buster" to the end of your CSS and JS files.</p>', 'castroytagle' );
			echo _e( 'To increment the cache buster version number, type something here:<br />', 'castroytagle' );
			echo '<input type="text" size="4" name="plugin_options[cache_buster_version]" value="'.$version.'"></p>';
			echo _e( '<p>Selecting this option will add the following code to the end of all of your CSS and JS file names on all of your pages:</p>', 'castroytagle' );
	...
	if ( ! function_exists( 'meta_tags_setting' ) ):
		function meta_tags_setting() {
			...
			echo _e( '<p>Add meta keywords tags for SEO, write your keywords separated by a comma, but not a space.</p>', 'castroytagle' );
			...
			echo _e('<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages, where </code>tags...</code> will be replaced with the code you insert above</p>', 'castroytagle' );
...
/*	4)	Create functions to add above elements to pages */
	...
	if ( ! function_exists( 'add_jquery_script' ) ):
		function add_jquery_script() {
			$cache = cache_buster();
			$options = get_option('plugin_options');
			$version = ($options['jquery_version']) ? $options['jquery_version'] : '1.9.1';
